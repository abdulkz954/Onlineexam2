...
function CheckNextPrevButtons() {
	...
	$("#btnPrevQue").removeAttr("disabled"), $("#btnNextQue").removeAttr("disabled"), e.is(":first-child") ? $("#btnPrevQue").attr("disabled", true) : e.is(":last-child") && $("#btnNextQue").attr("disabled", true)
}
...
function CoundownTimer(e) {
	...
	myInterval = setInterval(function() {
		myTimeSpan = 1e3 * t;
		$(".timer-title").text(GetTime(myTimeSpan)), t < 600 ? ($(".timer-title").addClass("time-ending pulsate"), $(".timer-title").removeClass("time-started")) : ($(".timer-title").addClass("time-started"), $(".timer-title").removeClass("time-ending pulsate")), t > 0 ? t -= 1 : CleartTimer()
	}, 1e3);
	// $(".pulsate").pulsate({
	// 	color: "#f00"
	// });
...
$(document).ready(function() {
	$("#page01").show();
	$(".exam-paper").show();
	CoundownTimer($("#remaining_duration").val());
	...
	$(".btn-save-answer").click(function(e) {
		...
		if ($("input[name='radios" + a + "']").each(function() {
			...
			toastr['warning']("Please choose an Option", "");
			return !1
		};
